...
ambient_ctrl = ambient_mod.Ambient()
mqtt_client = mqtt_mod.MQTT(io_status)
        ...
        try:
            ...
            if secs_elapsed >= task_at_secs['reiterate']:
                ...
                if config.MODULE_AMBIENT:
                    ambient_ctrl.ambient_redo()
                    io_status.set_ambient(ambient_ctrl)
            ...
            if secs_elapsed >= task_at_secs['update_io'] or is_status_changed or sighup_refresh:
                ...
                if config.MODULE_TRV:
                    mqtt_client.update_areas()
                if config.MODULE_HASS:
                    hass.publish_status(io_status, io_system, ambient_ctrl)
                # after a sighup refresh, reschedule task forward
...
def refresh_program(time_):
    ...
    if config.MODULE_TRV:
        ...
        for row in rows:
            ...
            # area registration
            if not registered:
                mqtt_client.register(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
...
def update_output():
    ...
    if current_status != io_status.get_output():
        dbmgr = db.DatabaseManager()
        io_status.current_image = resources.current_image(str(paths.resolve_under_data(config.IMAGE_PATH)))
            ...
            if config.MODULE_AMBIENT:
                try:
                    if not arg:
                        arg = 'COLOR'
                    if arg == 'COLOR':
                        ambient_ctrl.set_ambient_color(
                            value,
                            datetime.datetime.now() +
                            datetime.timedelta(hours=4))
                        ...
                        ambient_ctrl.set_ambient_color_hs(
                            value,
                            datetime.datetime.now() +
                            datetime.timedelta(hours=4))
                        ...
                        try:
                            ambient_ctrl.set_ambient_effect(arg, value,
                            datetime.datetime.now() +
                            datetime.timedelta(hours=4))
...
class HompiService:
    """Orchestrates I/O, climate, ambient lighting and the control loop."""
    ...
    def __init__(self):
        ...
        self.sensor = sensor
        self.lcd = lcd
        self.ambient_ctrl = ambient_ctrl
